...
# Model kayıt dizini
SAVE_DIR = 'efficientnet_b7_models'
...
# EfficientNetB7 için optimize edilmiş veri dönüşümleri
...
class EfficientNetB7Classifier(nn.Module):
    def __init__(self):
        ...
        self.model.classifier = nn.Sequential(
            nn.Dropout(p=0.5),  # B7 için daha yüksek dropout
...
train_loader = DataLoader(train_dataset, batch_size=8, shuffle=True)  # B7 için daha küçük batch size
...
# Early stopping
best_val_acc = 0
patience = 15  # B7 için daha uzun patience
patience_counter = 0

# Model oluşturma
model = EfficientNetB7Classifier().to(device)
criterion = nn.CrossEntropyLoss()
optimizer = torch.optim.AdamW(model.parameters(), lr=5e-5, weight_decay=2e-5)  # B7 için daha düşük learning rate
...
# Eğitim döngüsü
num_epochs = 20  # B7 için daha uzun eğitim
for epoch in range(num_epochs):
    try:
        ...
        model_save_path = os.path.join(SAVE_DIR, f'efficientnet_b7_epoch_{epoch+1}_acc_{val_acc:.4f}.pth')
        ...
        if val_acc > best_val_acc:
            ...
            best_model_path = os.path.join(SAVE_DIR, f'efficientnet_b7_best_acc_{val_acc:.4f}.pth')
        ...
        print("\nEğitim durduruldu!")
        interrupted_model_path = os.path.join(SAVE_DIR, f'efficientnet_b7_interrupted_epoch_{epoch+1}_acc_{val_acc:.4f}.pth')
