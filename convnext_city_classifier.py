...
# ConvNeXt için optimize edilmiş veri dönüşümleri
train_transform = transforms.Compose([
    transforms.Resize((224, 224)),  # ConvNeXt için standart boyut
...
train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)  # ConvNeXt için uygun batch size
...
for epoch in range(num_epochs):
    try:
        ...
        model_save_path = os.path.join(SAVE_DIR, f'convnext_epoch_{epoch+1}_acc_{val_acc:.4f}.pth')
        ...
        if val_acc > best_val_acc:
            ...
            best_model_path = os.path.join(SAVE_DIR, f'convnext_best_acc_{val_acc:.4f}.pth')
        ...
        print("\nEğitim durduruldu!")
        interrupted_model_path = os.path.join(SAVE_DIR, f'convnext_interrupted_epoch_{epoch+1}_acc_{val_acc:.4f}.pth')
